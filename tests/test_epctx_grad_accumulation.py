...
def _make_policy(seed: int = 0, num_envs: int = NUM_ENVS, memory_tokens: int = 0) -> EpisodeContextModel:
...
def _make_ppo(policy: EpisodeContextModel, num_envs: int = NUM_ENVS, **kwargs) -> EpisodeContextPPO:
    defaults = dict(
...
def test_default_is_the_single_pass_update() -> None:
    """``grad_accumulation_steps=1`` (the default) is one micro-batch per minibatch: same parameters, and the
    storage generator is asked for exactly one chunk per minibatch."""
    _, baseline = _run()
    ...
    ppo, _ = _run()
    assert ppo.grad_accumulation_steps == 1
    original = ppo.storage.recurrent_mini_batch_generator
    seen: list[int] = []

    def spy(num_mini_batches, num_epochs=8, grad_accumulation_steps=1):
        seen.append(grad_accumulation_steps)
        return original(num_mini_batches, num_epochs, grad_accumulation_steps)

    ppo.storage.recurrent_mini_batch_generator = spy  # type: ignore[method-assign]
    generator = torch.Generator(device=DEVICE).manual_seed(17)
    _collect(ppo, _episode_schedule(STEPS_PER_ENV), generator)
    ppo.update()
    assert seen == [1], f"G=1 asked the generator for {seen} micro-batches per minibatch"
    print("[ok] G=1 is the single-pass update; G>1 takes the accumulating path")
...
if __name__ == "__main__":
    test_default_is_the_single_pass_update()
