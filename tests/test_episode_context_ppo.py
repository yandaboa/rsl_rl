...
and the runner are untouched relative to the 5.2 base commit (everything lives in new files).
"""
...
GAMMA = 0.99
LAM = 0.95
BASE_COMMIT = "b8111fa"  # master: UW-Lab feature/manipulation 5.2.0 + gSDE/wandb fixes
    ...
    for normalization, label in ((False, "no obs normalization"), (True, "deferred obs normalization")):
        ...
        for rollout in range(3):
            ...
            for batch in ppo.storage.recurrent_mini_batch_generator(num_mini_batches=2, num_epochs=1):
                obs_batch, actions_batch = batch.observations, batch.actions
                ...
                reference = batch.old_actions_log_prob.squeeze(-1)
                max_error = max(max_error, (log_prob - reference).abs().max().item())
                value = policy.evaluate(obs_batch)
                max_value_error = max(max_value_error, (value - batch.values).abs().max().item())
