"""Acting vs update-path log-prob parity for EpisodeContextModel (CPU, no Isaac).
...
# Run from the rsl_rl checkout (``PYTHONPATH=. python tests/check_epctx_logprob_parity.py``).
sys.path.insert(0, __file__.rsplit("/tests/", 1)[0])

import torch.nn.functional as F  # noqa: E402

from rsl_rl.models import EpisodeContextModel, EpisodeContextPrefix  # noqa: E402
...
def build_policy(path: str):
    ...
    policy = EpisodeContextModel(example, obs_groups, num_actions, **pk).float()
    policy.load_state_dict(ck["model_state_dict"], strict=True)  # 3.1 checkpoint keys load unchanged
