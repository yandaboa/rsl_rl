...
def _make_policy(seed: int = 0, privileged: bool = True) -> EpisodeContextModel:
    torch.manual_seed(seed)
    policy = EpisodeContextModel(
        obs=_sample_obs(NUM_ENVS),
        obs_groups={"actor": ["policy", "privileged"] if privileged else ["policy"], "critic": ["critic"]},
...
def _priv_keys(policy: EpisodeContextModel) -> set[str]:
...
def test_bc_checkpoint_loads_with_only_privileged_keys_missing():
    ...
    # ``EpisodeContextModel.load_state_dict`` returns a bool, so go through nn.Module for the report.
...
def test_gradient_reaches_the_privileged_encoder():
    ...
    ppo = EpisodeContextPPO.create(
        policy,
        NUM_ENVS,
        STEPS_PER_ENV,
        _sample_obs(NUM_ENVS),
        [ACTION_DIM],
    ...
    # ``defer_obs_normalization`` is on by default: update() commits the rollout's frames, not per step.
