...
"""Tests for ``EpisodeContextModel(critic_design="separate_trunk")``.
...
def _make_ppo(policy: EpisodeContextModel, **kwargs) -> EpisodeContextPPO:
    ...
    ppo = EpisodeContextPPO.create(policy, NUM_ENVS, STEPS_PER_ENV, _sample_obs(), [ACTION_DIM], **defaults)
    return ppo
...
def _reference_values(
    policy: EpisodeContextModel, obs: torch.Tensor, dones: torch.Tensor, trial_dones: torch.Tensor
) -> torch.Tensor:
...
def test_value_loss_does_not_touch_the_actor() -> None:
    ...
    for memory_tokens in (0, 2):
        ...
        obs_batch, prefix = batch.observations, batch.hidden_states[1]
