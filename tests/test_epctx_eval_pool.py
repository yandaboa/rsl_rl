...
def _make_policy(seed: int = 0, noise_std_type: str = "scalar") -> EpisodeContextModel:
...
def test_eval_envs_act_on_the_mean() -> None:
    ...
    assert torch.equal(ppo.storage.actions[:, 6:], ppo.storage.distribution_params[0][:, 6:]), "stored eval rows are not the mean"
...
def test_update_never_sees_eval_rows() -> None:
    ...
    def spy(num_mini_batches: int, num_epochs: int = 8, grad_accumulation_steps: int = 1):
        for batch in original(num_mini_batches, num_epochs, grad_accumulation_steps):
...
def test_eval_envs_keep_the_kv_ring_in_sync() -> None:
    ...
    for rollout in range(3):
        ...
        mean_error = (policy.action_mean - ppo.storage.distribution_params[0]).abs().max().item()
