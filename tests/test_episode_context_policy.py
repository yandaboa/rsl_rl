...
from rsl_rl.models import EpisodeContextModel, EpisodeContextPrefix
...
def _roll_incrementally(policy: EpisodeContextModel, obs: torch.Tensor, dones: torch.Tensor) -> torch.Tensor:
...
def _window_hidden(
    policy: EpisodeContextModel, obs: torch.Tensor, positions: torch.Tensor, window_start: int
) -> torch.Tensor:
...
def test_window_is_episode_local_and_span_limited() -> None:
    ...
    (:attr:`EpisodeContextModel.context_prefix_length`), so it is asserted here rather than assumed.
    """
    ...
    for rollout in range(num_rollouts):
        for _ in range(window):
            ...
            transition.distribution_params = (
                ...
                torch.ones(num_envs, ACTION_DIM, device=DEVICE),
            )
