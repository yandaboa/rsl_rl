...
def _replay_log_probs(
    ...
    :meth:`EpisodeContextModel.forward_sequence` over its own frames, and the memory chain is rebuilt by
    ...
    for critic_design in ("privileged", "shared_trunk"):
        ...
        for rollout in range(3):
            ...
            for batch in ppo.storage.recurrent_mini_batch_generator(num_mini_batches=2, num_epochs=1):
                obs_batch, actions_batch, prefix = batch.observations, batch.actions, batch.hidden_states[0]
                ...
                policy.act(obs_batch, masks=batch.masks, hidden_state=prefix)
                log_prob = policy.get_actions_log_prob(actions_batch)
                max_error = max(max_error, (log_prob - batch.old_actions_log_prob.squeeze(-1)).abs().max().item())
...
def test_windowed_update_matches_a_from_scratch_replay() -> None:
    ...
    for rollout in range(2):
        ...
        with torch.no_grad():
            ...
            policy.act(batch.observations, masks=batch.masks, hidden_state=batch.hidden_states[0])
            windowed = policy.get_actions_log_prob(batch.actions)
...
def test_the_memory_is_load_bearing_in_the_update() -> None:
    ...
    with torch.no_grad():
        reference = policy.act(batch.observations, hidden_state=prefix).clone()
        ...
        perturbed = policy.act(batch.observations, hidden_state=wrecked)
        blind = type(prefix)(
            obs=prefix.obs,
            positions=prefix.positions,
            window_positions=prefix.window_positions,
        ...
        without_source = policy.act(batch.observations, hidden_state=blind)
...
def test_trial_boundary_cuts_the_source() -> None:
    ...
    prefix = next(ppo.storage.recurrent_mini_batch_generator(num_mini_batches=1, num_epochs=1)).hidden_states[0]
    num_prefix = prefix.obs.shape[0]
...
def test_generator_gradients_reach_the_memory_modules() -> None:
    ...
    policy.act(batch.observations, masks=batch.masks, hidden_state=prefix)
    ratio = torch.exp(policy.get_actions_log_prob(batch.actions) - batch.old_actions_log_prob.squeeze(-1))
    value = policy.evaluate(batch.observations)
    loss = -(ratio * batch.advantages.squeeze(-1)).mean() + (value - batch.returns).pow(2).mean()
    loss.backward()
...
def test_memory_tokens_zero_is_the_stock_step() -> None:
    """Without a memory (and without a trial signal), ``process_env_step`` must match the stock 5.2 ``PPO`` step
    transition for transition, and the generator must carry no memory."""
    dones, _ = _trial_schedule(STEPS_PER_ENV)
    stored = []
    for use_override in (True, False):
        ...
        prefix = next(ppo.storage.recurrent_mini_batch_generator(num_mini_batches=2, num_epochs=1)).hidden_states[0]
